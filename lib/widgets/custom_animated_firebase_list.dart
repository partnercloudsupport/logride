...
// Based off of the original firebase_animated_list in the firebase_database
// flutter plugin. Modified by Thomas Stoeckert (Thomas@Stoeckert.us) to include
// access to total module length as well as to include an empty child, based off
// of https://github.com/flutter/plugins/pull/256
...
class FirebaseAnimatedList extends StatefulWidget {
  ...
  /// A widget to display when the query is empty. Defaults to an empty
  /// [Container].
...
class FirebaseAnimatedListState extends State<FirebaseAnimatedList> {
  ...
  void _onChildAdded(int index, DataSnapshot snapshot) {
    ...
    // If the list was previously empty, the current state may behave oddly/act
    // like it's null. We just need to be safe, so adding the ? makes it good.
